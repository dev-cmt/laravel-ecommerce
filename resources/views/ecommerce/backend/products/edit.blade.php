<x-app-layout :title="'Edit Product'">
    @push('style')
        <style>
            /* Your custom styles from the create page can be reused */
        </style>
    @endpush
    ...
    <div class="container">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="sku_code" id="sku_code" class="form-control @error('sku_code') is-invalid @enderror" value="{{ old('sku_code', $product->sku_code) }}">
                @error('sku_code')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="url_slug" id="url_slug" class="form-control @error('url_slug') is-invalid @enderror" value="{{ old('url_slug', $product->url_slug) }}" required>
                @error('url_slug')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                @if($product->img_path)
                    <p>Current Image: <img src="{{ asset('storage/' . $product->img_path) }}" alt="Product Image" width="100"></p>
                @endif
                @error('img_path')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                    ...
                    @foreach($product->images as $index => $image)
                        <div class="image">
                            <div class="form-group">
                                ...
                                @if($image->image_path)
                                    <p>Current Image: <img src="{{ asset('storage/' . $image->image_path) }}" alt="Product Image" width="100"></p>
                                @endif
                            </div>
                            <button type="button" class="btn btn-danger remove-image">Remove</button>
                        </div>
                ...
                <div id="details">
                    @foreach($product->details as $index => $detail)
                        <div class="detail">
                            <div class="form-group">
                                <label for="detail_name">Detail Name</label>
                                <input type="text" name="details[{{ $index }}][detail_name]" class="form-control" value="{{ $detail->detail_name }}" required>
                            </div>
                            <div class="form-group">
                                <label for="detail_value">Detail Value</label>
                                <textarea name="details[{{ $index }}][detail_value]" class="form-control" required>{{ $detail->detail_value }}</textarea>
                            </div>
                            <button type="button" class="btn btn-danger remove-detail">Remove</button>
                        </div>
                    @endforeach
                ...
                <div id="specifications">
                    @foreach($product->specifications as $index => $specification)
                        <div class="specification">
                            <div class="form-group">
                                <label for="specification_name">Specification Name</label>
                                <input type="text" name="specifications[{{ $index }}][specification_name]" class="form-control" value="{{ $specification->specification_name }}" required>
                            </div>
                            <div class="form-group">
                                <label for="specification_value">Specification Value</label>
                                <textarea name="specifications[{{ $index }}][specification_value]" class="form-control" required>{{ $specification->specification_value }}</textarea>
                            </div>
                            <button type="button" class="btn btn-danger remove-specification">Remove</button>
                        </div>
                    @endforeach
