                               ...
                               <div class="tp-shop-widget-categories">
                                   <ul>
                                    @foreach ($categories as $item)
                                       <li><a href="#">{{ $item->category_name}} <span>{{$item->products->count()}}</span></a></li>
                                    @endforeach
                                   </ul>
                               </div>
                           </div>
                                        ...
                                        @foreach ($categories as $category)
                                            <li>
                                                <div class="tp-shop-widget-checkbox-circle">
                                                    ...
                                                    <span data-bg-color="#54sdfd" class="tp-shop-widget-checkbox-circle-self"></span>
                                                </div>
                       ...
                       @push('scripts')
                        <script>
                            $(document).ready(function () {
                                ...
                                // Function to get selected filter values and send an AJAX request
                                function filterProducts() {
                                    let priceRange = $("#amount").val(); // Assuming this contains the price range
                                    let selectedCategories = [];
                                    let selectedColors = [];

                                    // Get selected categories
                                    $(".category-filter:checked").each(function () {
                                        selectedCategories.push($(this).attr('data-id')); // Extracting ID from 'category-<id>'
                                    });

                                    // Get selected colors
                                    $(".color-filter:checked").each(function () {
                                        selectedColors.push($(this).attr('data-id')); // Extracting ID from 'color-<id>'
                                    });

                                    // Send AJAX request to server to fetch filtered products
                                    $.ajax({
                                        url: "{{ route('filter-products') }}", // Laravel route to handle filter request
                                        method: "GET",
                                        data: {
                                            price: priceRange,
                       ...
                       <div class="tp-shop-items-wrapper tp-shop-item-primary">
                           <div class="tab-content" id="productTabContent">
                               <div class="tab-pane fade show active" id="grid-tab-pane" role="tabpanel" aria-labelledby="grid-tab" tabindex="0">
                                   <div class="row infinite-container" id="product-list-container">
